...
export const IncidentsWidget: React.FC<IncidentsWidgetProps> = ({ state, sendEvent }) => {
  ...
  const getPriorityBadge = (prio: number) => {
    if (prio === 1) {
      return <span className="px-1.5 py-0.5 text-[10px] font-bold bg-red-950 text-red-300 border border-red-700 rounded">PRIO 1</span>;
    }
    if (prio === 2) {
      return <span className="px-1.5 py-0.5 text-[10px] font-bold bg-yellow-950 text-yellow-300 border border-yellow-700 rounded">PRIO 2</span>;
    }
    return <span className="px-1.5 py-0.5 text-[10px] font-bold bg-blue-950 text-blue-300 border border-blue-700 rounded">PRIO 3</span>;
  };

  const getStatusBadge = (status: Incident['status']) => {
    switch (status) {
      case 'Gemeldet':
        return <span className="text-[10px] text-amber-400 flex items-center gap-1"><Clock className="w-2.5 h-2.5" /> Gemeldet</span>;
      case 'In Bearbeitung':
        return <span className="text-[10px] text-cyan-400 flex items-center gap-1"><Clock className="w-2.5 h-2.5" /> In Bearbeitung</span>;
      case 'Unter Kontrolle':
        return <span className="text-[10px] text-emerald-400 flex items-center gap-1"><Check className="w-2.5 h-2.5" /> Unter Kontrolle</span>;
      default:
      ...
      <div className="flex-1 overflow-y-auto custom-scroll space-y-2 pr-1">
        {activeIncidents.length === 0 ? (
          <div className="h-full min-h-[140px] flex flex-col items-center justify-center text-center p-4 border border-dashed border-slate-800 rounded-lg">
            ...
            <p className="text-xs font-semibold text-slate-300">Keine aktiven Einsatzstellen</p>
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  {inc.description && (
                    <p className="text-[11px] text-slate-400 mt-0.5 line-clamp-2">{inc.description}</p>
                  )}
                </div>
                ...
                <div className="flex flex-col items-end gap-1.5 shrink-0">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 px-2 py-0.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded text-[10px] font-semibold transition-colors"
