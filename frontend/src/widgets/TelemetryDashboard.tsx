...
export const TelemetryDashboard: React.FC<WidgetProps> = ({ telemetry, theme }) => {
  ...
  const getDangerBadge = (danger: number, status: string) => {
    if (danger >= 2) {
      return <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-red-950 text-red-300 border border-red-700 animate-pulse">{status}</span>;
    }
    if (danger === 1) {
      return <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-amber-950 text-amber-300 border border-amber-700">{status}</span>;
    }
    return <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-950/80 text-emerald-300 border border-emerald-800">{status}</span>;
  };
      ...
      <div className="flex-1 min-h-0 flex flex-col gap-3 mt-3 overflow-hidden">
        ...
        <div className="flex-[5] flex flex-col kats-panel-sub rounded-lg p-2.5 overflow-hidden">
          <div className="flex items-center justify-between pb-1.5 mb-1.5 border-b border-slate-800/80">
            ...
            <span className="text-[10px] text-slate-400 font-mono">cm / 1h-Tendenz</span>
          </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scroll space-y-2 pr-1">
            {live.water_levels && live.water_levels.length > 0 ? (
              live.water_levels.map((pegel, idx) => (
                <div key={idx} className="bg-slate-900/80 p-2.5 rounded-lg border border-slate-800/90 flex flex-col gap-1.5">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-xs text-slate-100">{pegel.station}</span>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1 text-xs font-mono bg-slate-950 px-2 py-0.5 rounded border border-slate-800">
              ...
              <div className="text-center py-6 text-xs text-slate-500">Keine Pegeldaten geladen</div>
            )}
          </div>
        </div>
        ...
        <div className="flex-[4] flex flex-row gap-2.5 min-h-0 overflow-hidden">
          ...
          <div className="flex-[2] kats-panel-sub rounded-lg p-2.5 flex flex-col justify-between overflow-hidden">
            <div className="flex items-center justify-between pb-1 border-b border-slate-800/80">
              ...
              <span className="text-[10px] text-slate-400 font-mono">Live-Messwerte</span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 my-1">
              <div className="bg-slate-900/80 p-2 rounded border border-slate-800">
                ...
                <div className="mt-1">
                  <span className="text-base font-extrabold font-mono text-slate-100">
                    {live.weather.wind_speed_kmh} <span className="text-xs font-normal text-slate-400">km/h</span>
                  </span>
              ...
              <div className="bg-slate-900/80 p-2 rounded border border-slate-800">
                ...
                <div className="mt-1">
                  ...
                  <div className="text-[11px] font-mono text-slate-400 flex items-center gap-0.5">
              ...
              <div className="bg-slate-900/80 p-2 rounded border border-slate-800">
                ...
                <div className="mt-0.5">
                  <span className="text-sm font-bold font-mono text-slate-100">
                    {live.weather.precipitation_mm} <span className="text-xs font-normal text-slate-400">mm/h</span>
                  </span>
              ...
              <div className="bg-slate-900/80 p-2 rounded border border-slate-800">
                <span className="text-[10px] text-slate-400">DWD Akut-Warnlage</span>
                <div className="mt-0.5">
          ...
          <div className="flex-1 kats-panel-sub rounded-lg p-2.5 flex flex-col justify-between overflow-hidden">
            ...
            <div className="flex-1 flex flex-col justify-center items-center py-2">
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-5xl font-black font-mono text-red-500 drop-shadow-md">
                  {live.fire_missions_yesterday !== undefined ? live.fire_missions_yesterday : "--"}
                </span>
                <span className="text-sm font-bold text-slate-400">Einsätze</span>
              </div>
