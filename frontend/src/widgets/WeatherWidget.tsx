          ...
          <div className="flex flex-col gap-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-white dark:bg-slate-900/80 p-2 rounded-lg border border-slate-200 dark:border-slate-800 flex flex-col justify-center items-center">
                <span className="text-[10px] text-slate-500 flex items-center gap-1.5 mb-1">
                ...
                <span className="text-xl font-extrabold font-mono text-slate-800 dark:text-slate-100">
                  {live.wind_speed_kmh} <span className="text-[10px] font-normal text-slate-500">km/h</span>
                </span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-slate-50 dark:bg-slate-800/50 p-2 rounded-lg border border-slate-200 dark:border-slate-700 flex flex-col justify-center items-center">
                ...
                <span className="text-lg font-mono font-bold text-slate-800 dark:text-slate-100">
                  {live.air_pressure_hpa} <span className="text-[10px] font-normal text-slate-500">hPa</span>
                </span>
              </div>
              ...
              <div className="bg-slate-50 dark:bg-slate-800/50 p-2 rounded-lg border border-slate-200 dark:border-slate-700 flex flex-col justify-center items-center text-center">
                ...
                <span className="text-lg font-mono font-bold text-slate-800 dark:text-slate-100 leading-none">
                  {live.wind_direction}
                </span>
                <div className="text-[9px] text-slate-500 mt-1">Böen bis {live.wind_gusts_kmh} km/h</div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900/80 px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-800 flex items-center justify-between">
               <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    <span className="text-[10px] text-slate-500">Niederschlag</span>
            ...
            <div className={`p-3 rounded-lg border ${
              ...
              <span className="text-[11px] font-bold flex items-center gap-1.5 mb-1 text-slate-800 dark:text-slate-200">
          ...
          {next7Days.map((day: DayForecast, i: number) => (
            <div key={i} className="flex-shrink-0 flex items-center justify-between px-4 py-3 min-h-[64px] bg-white dark:bg-slate-900/60 rounded-lg border border-slate-200 dark:border-slate-800/80 shadow-sm">
              <div className="flex flex-col min-w-[70px]">
                <span className="font-bold text-sm text-slate-800 dark:text-slate-200 leading-tight">{day.weekday}</span>
                <span className="text-[10px] text-slate-500 font-mono leading-tight mt-0.5">{day.date.split('-').slice(1).join('.')}</span>
              </div>
              
              <div className="flex flex-col items-center flex-1 px-2">
                 <span className="text-xs text-slate-600 dark:text-slate-300 leading-tight truncate w-full text-center font-medium">{day.condition}</span>
                 {day.precipitation_prob > 20 && (
                   <span className="text-[10px] text-blue-500 flex items-center gap-1 leading-tight mt-1 font-semibold">
                     <CloudRain className="w-3 h-3" /> {day.precipitation_prob}%
                   </span>
                 )}
              </div>

              <div className="flex items-center gap-2 font-mono text-sm min-w-[80px] justify-end">
                <span className="text-blue-500 dark:text-blue-300 font-medium">{day.temp_min}°</span>
                <span className="text-slate-300 dark:text-slate-600">/</span>
                <span className="text-red-500 dark:text-red-400 font-bold text-base">{day.temp_max}°</span>
              </div>
            </div>
          ))}
          {next7Days.length === 0 && (
            <div className="text-slate-500 text-center py-4 text-sm flex items-center justify-center">Lade Vorhersage...</div>
          )}
        </div>
      </WidgetContainer>
    </div>
